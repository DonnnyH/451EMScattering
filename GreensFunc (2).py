...
def ScatteringMatrix(R, l, p, w0):
    ...
    for i in range(3*n):
        ...
        E[i]=(LG_field(r,z,theta,l,p,w0))*polarisation(i,0)
    for i in range(3*n):
        for j in range(3*n):
...
def LG_field_Scat(x,y,z, E, R, l, p, w0):
    """
    Computes the field after the scattering solution has been solved by including the induced fields of the dipoles
    """
    E0 = np.array([[LG_field(np.sqrt(x**2+y**2),z,np.arctan2(y,x), l, p, w0)],[0],[0]], dtype=complex)
    q = np.array([x,y,z])
    if len(R)==0:
        E0=E0
    else:
...
def farfield(theta, phi, E, R, l, p, w0):
    def normvect(theta, phi):
        """
    ...
    r = 1200 #far_field radius
    x = r*np.sin(theta)*np.cos(phi)
    y = r*np.sin(theta)*np.sin(phi)
    z= r*np.cos(theta)
    Ef = np.dot(normvect(theta, phi), LG_field_Scat(x, y, z, E, R, l, p, w0))
    return Ef

def Re_farfield(theta, phi, E, R, l, p, w0):
    return np.real(farfield(theta, phi, E, R, l, p, w0))*np.sin(theta)

def Im_farfield(theta, phi, E, R, l, p, w0):
    return np.imag(farfield(theta, phi, E, R, l, p, w0))*np.sin(theta)    
    
def Transmitted(R, l, p, w0):
    Escat = ScatteringMatrix(R, l, p, w0)
    def farfield(theta, phi):
        def normvect(theta, phi):
            """
            Unit Vector of normal to sphere
            thet - azimuthal angle [0,2*pi)
        ...
        y = r*np.sin(theta)*np.sin(phi)
        z = r*np.cos(theta)
        Ef = np.dot(normvect(theta, phi), LG_field_Scat(x, y, z, Escat, R, l, p, w0))
        return Ef
    def Re_farfield(theta, phi):
        return np.real(farfield(theta, phi))*np.sin(theta)
    def Im_farfield(theta, phi):
        return np.imag(farfield(theta, phi))*np.sin(theta)
    I1 = scipy.integrate.dblquad(Re_farfield,0, 2*np.pi, 0, np.pi/2)[0]
    I2 = scipy.integrate.dblquad(Im_farfield,0, 2*np.pi, 0, np.pi/2)[0]
    I = I1+1j*I2
    return np.abs(I)**2

def Transmitted_0(l,p,w0):
    def farfield(theta, phi):
        r = 1200 #far_field radius
        x = r*np.sin(theta)*np.cos(phi)
        y = r*np.sin(theta)*np.sin(phi)
        z = r*np.cos(theta)
        Ef = LG_field(np.sqrt(x**2+y**2), z, np.arctan2(y,x), l, p, w0)*np.sin(theta)*np.cos(phi)
        return Ef
    def Re_farfield(theta, phi):
        return np.real(farfield(theta, phi))*np.sin(theta)
    def Im_farfield(theta, phi):
...
for i in range(m):
    z = -10
    for j in range(m):
        I=(np.linalg.norm(LG_field_Scat(x,0, z,Escat, R,l, p, w0))**2)
        pos[i][j]=I
        z = z+ 20/m
    x = x + 20/m
